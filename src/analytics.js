...
// ---------- merging (cloud sync and backup codes never delete, they only add) ----------
function mergeProgressMaps(into, from) {
  Object.entries(from || {}).forEach(([id, p]) => {
    if (!p) return;
    const cur = into[id], incoming = { ...p, streak: p.streak ?? Math.min(p.solved || 0, 3) };
    if (!cur || (incoming.tries || 0) > (cur.tries || 0)) into[id] = incoming;
  });
}
function mergeState(data) {
  let added = 0;
  const ids = new Set(state.log.map((e) => e.id));
  (data.log || []).forEach((e) => { if (e && e.id && !ids.has(e.id)) { state.log.push(e); ids.add(e.id); added++; } });
  state.log.sort((a, b) => a.t - b.t);
  const tids = new Set(state.tests.map((t) => t.id));
  (data.tests || []).forEach((t) => { if (t && t.id && !tids.has(t.id)) { state.tests.push(t); tids.add(t.id); added++; } });
  state.tests.sort((a, b) => a.t - b.t);
  mergeProgressMaps(state.progress, data.progress);
  mergeProgressMaps(state.pprogress, data.pprogress);
  Object.entries(data.mockScores || {}).forEach(([k, v]) => { state.mockScores[k] = Math.max(state.mockScores[k] ?? 0, v); });
  Object.entries(data.plan || {}).forEach(([k, v]) => { if (v) state.plan[k] = true; });
  return added;
}

const STORAGE_OK = (() => {
  try { const k = "mat213-storage-test"; localStorage.setItem(k, "1"); const ok = localStorage.getItem(k) === "1"; localStorage.removeItem(k); return ok; } catch (e) { return false; }
})();
function renderSaveBar() {
  ...
  else if (!STORAGE_OK) { warn = true; msg = "⚠ This browser isn't saving your progress (private window or blocked site data). Before closing this tab, copy a backup code in My gaps."; }
  else if (st === "pending") msg = "Progress saves in this browser. Connecting cloud sync…";
  else if (host === "pages") msg = "Progress saves in this browser only. To move it, use a backup code in My gaps, or practice on the claude.ai link, which syncs.";
  else if (host === "file") msg = "Desktop copy: progress saves in this browser only. The claude.ai link syncs across devices.";
  else if (st === "error" || st === "full") { warn = true; msg = "⚠ Cloud sync hit a problem. Progress is still saved in this browser."; }
  else msg = "Progress saves in this browser only (cloud sync isn't available in this view).";
  bar.className = `savebar${warn ? " warn" : ""}`;
  bar.textContent = msg;
}

// ---------- cloud sync (only inside the published page; the local copy keeps data in the browser) ----------
const Sync = (() => {
  let db = null, ready = false, timer = null, status = window.claude && typeof window.claude.use === "function" ? "pending" : "local";
  const CHUNK = 100;
  ...
  async function init() {
    ...
    try {
      ...
      if (pg.exists) { const body = pg.data(); mergeState({ progress: body.progress, pprogress: body.pprogress, mockScores: body.mockScores, plan: body.plan }); }
  ...
  async function flush() {
    try {
      ...
      await db.doc("analytics/progress").set({ progress: JSON.parse(JSON.stringify(state.progress)), pprogress: JSON.parse(JSON.stringify(state.pprogress)), mockScores: { ...state.mockScores }, plan: { ...state.plan }, updated: Date.now() });
...
const syncText = (s) => ({
  ...
  pending: "Connecting to the page's storage…",
})[s];
...
function renderStats() {
  ...
  const backup = el("section", { class: "sheet" });
  backup.innerHTML = `<div class="sheet-head"><h3>Back up or move your progress</h3><span class="tag">phone ↔ computer</span></div>
    <div class="prompt"><p>Each link keeps its own progress: the claude.ai link syncs, while the phone link and the Desktop file save only in that browser. To move progress, copy a backup code on one and restore it on the other. Restoring only adds records; it never deletes any.</p></div>`;
  const code = el("textarea", { class: "report-box", id: "backup-box", "aria-label": "backup code", placeholder: "Paste a backup code here, then choose Restore" });
  code.style.minHeight = "90px";
  const bmsg = el("span", { class: "score-line", "aria-live": "polite" }, restoreNote);
  restoreNote = "";
  const makeCode = () => btoa(unescape(encodeURIComponent(JSON.stringify({ v: 1, log: state.log, tests: state.tests, progress: state.progress, pprogress: state.pprogress, mockScores: state.mockScores, plan: state.plan }))));
  const bcopy = btn("Copy backup code", "primary", async () => {
    code.value = makeCode();
    try { await navigator.clipboard.writeText(code.value); bmsg.textContent = "Backup code copied."; }
    catch (e) { code.focus(); code.select(); try { document.execCommand("copy"); bmsg.textContent = "Backup code copied."; } catch (e2) { bmsg.textContent = "Selected. Press Ctrl+C to copy it."; } }
  });
  const brestore = btn("Restore from code", "", () => {
    let data;
    try { data = JSON.parse(decodeURIComponent(escape(atob(code.value.trim())))); } catch (e) { bmsg.textContent = "That code couldn't be read. Copy the whole code and try again."; return; }
    const added = mergeState(data);
    persist(); Sync.touch();
    restoreNote = `Restored: ${added} new answers and tests merged in, and your dots are updated.`;
    renderRail(); render();
  });
  const bacts = el("div", { class: "actions" });
  bacts.append(bcopy, brestore, bmsg);
  backup.append(code, bacts);
  main.append(rep, backup);
}
