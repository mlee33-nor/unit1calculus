...
// ---------- practice tests ----------
// Tests keep each question short: at most two answer boxes (the key ones)
const KEEP_PARTS = {
  limGraph: () => [2, R.pick([0, 1, 3])],
  limInfinite: () => [2, R.pick([0, 1])],
  graphRead: (n) => (n >= 8 ? [R.int(0, 3), R.int(4, 7)] : [0, n - 1]),
};
function shortenProblem(pr, id, max = 2) {
  const n = pr.parts.length;
  if (n <= max) return pr;
  const keep = [...new Set((KEEP_PARTS[id] ? KEEP_PARTS[id](n) : [0, n - 1]).filter((i) => i >= 0 && i < n))].sort((a, b) => a - b);
  return { ...pr, parts: keep.map((i) => pr.parts[i]) };
}
function buildPTestProblems(test) {
  const full = !!(state.examCfg && state.examCfg.full);
  return withSeed(test.seed, () => (test.blueprint || PTEST_BLUEPRINT).map((id) => { const pr = makePProblem(id); return { id, pr: full ? pr : shortenProblem(pr, id) }; }));
}
const answerCount = (problems) => problems.reduce((a, p) => a + p.pr.parts.length, 0);
...
function renderPTestHome() {
  ...
  intro.innerHTML = `<div class="sheet-head"><h3>Pearson-style practice tests</h3><span class="tag">§1.1 – §1.4 · ${PTEST_BLUEPRINT.length} questions</span></div>
    ...
    <div class="setup">${minutesSelect("pt-min")}<label for="pt-n">Questions <select id="pt-n"><option value="10">10</option><option value="15">15</option><option value="20">20</option><option value="25">25</option></select></label><label for="pt-calc">Calculator <select id="pt-calc"><option value="yes">Allowed</option><option value="no">Not allowed</option></select></label><label for="pt-len">Length <select id="pt-len"><option value="quick">Quick (1–2 answers per question)</option><option value="full">Full (every part)</option></select></label><span class="note">Match these to your real exam once you know the details.</span></div>`;
  main.append(intro);
  ...
  document.getElementById("pt-len").value = cfg.full ? "full" : "quick";
  ["pt-min", "pt-n", "pt-calc", "pt-len"].forEach((id) => document.getElementById(id).addEventListener("change", () => {
    state.examCfg = { minutes: +document.getElementById("pt-min").value, n: +document.getElementById("pt-n").value, calc: document.getElementById("pt-calc").value === "yes", full: document.getElementById("pt-len").value === "full" };
    persist();
    renderPTestHome();
  ...
  PTESTS.forEach((t, i) => {
    const best = state.mockScores[t.key], probs = buildPTestProblems({ ...t, blueprint: testBlueprint() });
    const card = el("section", { class: "sheet mock-card" });
    card.innerHTML = `<div class="mock-letter" aria-hidden="true">${i + 1}</div><div><h3>${t.name}</h3><p class="mock-meta">${probs.length} questions · ${answerCount(probs)} answers · same every time</p><p class="mock-best">${best != null ? `Best score <b>${best}%</b>` : "Not taken yet"}</p></div>`;
...
function runPTest(test, minutes) {
  clearInterval(examTimer);
  const main = document.getElementById("main"); main.innerHTML = "";
  const problems = buildPTestProblems(test);
  const bar = el("div", { class: "exam-bar" });
  bar.innerHTML = `<span class="clock" id="clock">${minutes ? `${minutes}:00` : "∞"}</span><span class="meta">${test.name} · ${answerCount(problems)} answers<br><span class="pt-pos">Question 1 of ${problems.length}</span></span>`;
