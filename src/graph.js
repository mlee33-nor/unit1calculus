...
const Graph = (() => {
  ...
  function frame(o) {
    ...
    if (!o.edgeLabels) parts.push(`<text class="gp-lab" x="${yAxisX - 6}" y="${xAxisY + 15}" text-anchor="end">0</text>`);
    ...
    return {
      ...
      curve(fn, a, b, n = 320) {
        ...
        parts.push(`<path class="gp-curve" clip-path="url(#${id})" d="${d}"/>`);
      },
      ...
      dot(x, y, filled) { top.push(`<circle class="${filled ? "gp-closed" : "gp-open"}" cx="${X(x)}" cy="${Y(y)}" r="${filled ? 4.4 : 4.6}"/>`); },
  ...
  function drawPiecewise(nodes, view, label) {
    ...
    nodes.slice(1, -1).forEach((nd) => {
      ...
      if (L === Rr) { if (v !== L) { g.dot(nd.x, L, false); if (v != null) g.dot(nd.x, v, true); } return; }
  ...
  const SLOPES = [-1.5, -1, -0.5, 0, 0.5, 1, 1.5];
  // types: array like ["jump","hole","cont","corner","asym"]
  function randomPiecewise(types, opts = {}) {
    const view = { xmin: -6, xmax: 6, ymin: -5, ymax: 5, W: 400, H: 340 };
    for (let attempt = 0; attempt < 400; attempt++) {
      const n = types.length;
      // spaced integer positions
      let xs = [];
      const gapMin = n >= 4 ? 2 : 3;
      for (let t = 0; t < 50 && xs.length < n; t++) {
        xs = [];
        let cur = -5 + R.int(0, 1);
        for (let i = 0; i < n; i++) { xs.push(cur); cur += gapMin + R.int(0, n >= 4 ? 1 : 2); }
        if (xs[n - 1] > 5) xs = [];
      }
      ...
      if (bad) continue;
      const nodes = [{ x: view.xmin - 0.6, yR: R.int(-3, 3), sR: R.pick(SLOPES) }];
      ts.forEach((t, i) => {
        const y = R.int(-3, 4), s = R.pick(SLOPES), nd = { x: xs[i], t };
      ...
      nodes.push({ x: view.xmax + 0.6, yL: R.int(-3, 3), sL: R.pick(SLOPES) });
      // validate ranges
      let ok = true;
      for (let i = 0; i < nodes.length - 1 && ok; i++) {
        ...
        for (let k = 1; k < 60; k++) {
          ...
          if (fin && (y > 4.7 || y < -4.7)) { ok = false; break; }
        }
      }
      nodes.forEach((nd) => { if (nd.val != null && (nd.val > 4.5 || nd.val < -4.5)) ok = false; if (isFinite(nd.yR) && (nd.yR > 4.5 || nd.yR < -4.5)) ok = false; });
      if (!ok) continue;
      ...
      return { nodes, svg: drawPiecewise(nodes, view, "piecewise graph"), inner: nodes.slice(1, -1).map(info) };
    }
  ...
  return { frame, randomPiecewise, drawPiecewise, lineChart, fnPlot, info };
})();
